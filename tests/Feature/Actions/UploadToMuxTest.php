...
use Daun\StatamicMux\Actions\UploadToMux;
...
test('visible to mux assets that are not yet uploaded', function () {
    $action = new UploadToMux;

    expect($action->visibleTo(MuxAsset::fromAsset($this->mp4)))->toBeTrue();
});

test('not visible to uploaded or proxy assets', function () {
    $action = new UploadToMux;

    expect($action->visibleTo((new MuxAsset([], $this->mp4))->withId('mux-asset-001')))->toBeFalse();
    expect($action->visibleTo((new MuxAsset([], $this->mp4))->withProxy()))->toBeFalse();
    expect($action->visibleTo('not-a-mux-thing'))->toBeFalse();
});

test('run queues an upload job for each asset', function () {
    $result = (new UploadToMux)->run(collect([MuxAsset::fromAsset($this->mp4)]), []);
    ...
    Queue::assertPushed(CreateMuxAssetJob::class, function ($job) {
        ...
        return $asset->id() === $this->mp4->id() && $force === false;
    });

    expect($result['callback'])->toBe(['pollMuxMirroredAssetRows', [$this->mp4->id()], 'upload']);
});
